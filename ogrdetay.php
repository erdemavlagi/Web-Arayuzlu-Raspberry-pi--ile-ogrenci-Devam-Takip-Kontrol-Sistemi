	...
	?>
		<center>
			...
			<div class="card-header" style="color:#FFFFFF; background-color: #D90000;">
				<div class="row">
					<div class="col text-start">Öğrenci Bilgileri</div>
		...
		<div class="row">
			...
			<div class="col-6">
			<?php
				$KisiKartSql = "SELECT * FROM kisi_kart WHERE kisi_no = '".$_GET["KN"]."' ORDER BY kart_durum";
				$KisiKartSorguSonucu = $db->query($KisiKartSql); // Tablodaki tüm verileri çekiyoruz.
				if($KisiKartSorguSonucu)							
				{
					$SiraNo = 0;	
				?>
				<button type="button" class="btn btnekle " id="btnkartekle">+ Yeni Kart Ekle</button>
				<table class="table" id="kartlar" cellpadding="0" cellspacing="0" border="0" style="background-color: #4D4D4D; color: #FFFFFF;border-collapse: collapse; border-spacing: 0; font-weight: bold;">
				  <thead>
					<tr style="background-color: #D90000; color: #FFFFFF;">
					  <th scope="col">Sıra</th>
					  <th scope="col" class="text-center">Kart No</th>
					  <th scope="col" class="text-center">Kart Durum</th>
					</tr>
				  </thead>
				  <tbody>
				<?php		
					while ($kart = $KisiKartSorguSonucu->fetch_assoc()) 
					{						
						//Aktif kart bilgisi
						if ($kart['kart_durum'] =="E") 
						{
							$kartDurum = "Aktif";
						}
						else
						...
						echo "<tr>";
							echo "<td>".$SiraNo."</td>";
							echo "<td class='text-center'>".$kart['kart_no']."</td>";
							echo "<td class='text-center'>".$kartDurum."</td>";
						echo "</tr>";	
					}
				?>		
				  </tbody>
		...
		<div class="row">
			...
			<div class="col-8">
			<?php
				$KisiYoklamaSql = "SELECT * FROM yoklama_log Y, kisi_kart K
									WHERE
									Y.yoklama_kartno=K.kart_no
									AND K.kisi_no = '".$_GET["KN"]."'
				...
				<table class="table" id="yoklamalar" cellpadding="0" cellspacing="0" border="0" style="background-color: #4D4D4D; color: #FFFFFF;border-collapse: collapse; border-spacing: 0; font-weight: bold;">
				  <thead>
					<tr style="background-color: #D90000; color: #FFFFFF;">
					  ...
					  <th scope="col">Terminal(Sınıf)</th>
				...
				<?php		
					...
					{
				//Tarih Karşılaştırma
						if ($yoklama['yoklama_tarih'] > $Bugun) 
						{
							$YoklamaRenk = "#009D03";
						}
						else
						...
						$SiraNo ++;	
						echo "<tr style='background-color:".$YoklamaRenk ."; '>";
							echo "<td>".$SiraNo."</td>";
							echo "<td>".$yoklama['yoklama_kartno']."</td>";
							echo "<td>".$yoklama['yoklama_tip']."</td>";
							echo "<td>".$yoklama['yoklama_tarih']."</td>";
						echo "</tr>";	
					}
				?>		
				  </tbody>
	...
	<div class="modal-content">
		<form class="form-horizontal" id="kaydetOGRFRM">
			  <div style="background-color:#D90000; color:white;" class="modal-header">
				<h5 class="modal-title" id="myModalLabel">Öğrenci Bilgisi Düzenleme</h5>
			  </div>
			<div class="modal-body">
				<div class="row">
					<div class="col-12">
					  <input type="text" class="form-control" id="yeni_OGRNO" name="yeni_OGRNO"  placeholder="Öğrenci No" autocomplete="off" maxlength="10" required readonly>
				...
				<input type="hidden" id="islemTip" name="islemTip" value="ogrGuncelle">	
...
<!-- Kart Ekle Modal -->
<div class="modal fade modal-sm" id="KartEkleModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-dialog-centered" role="document">
	<div class="modal-content">
		<form class="form-horizontal" id="kaydetKARTFRM">
			  <div style="background-color:#D90000; color:white;" class="modal-header">
				<h5 class="modal-title" id="myModalLabel">Yeni Kart Bilgileri</h5>
			  </div>
			<div class="modal-body">
				<div class="row">
					<div class="col-12">
					  <input type="text" class="form-control" id="yeni_KARTNO" name="yeni_KARTNO"  placeholder="Kart No" autocomplete="off" maxlength="20" required>
					</div>	
				</div>
				<br>	
				<input type="hidden" id="islemTip" name="islemTip" value="kartEkle">
				<input type="hidden" id="kisiNO" name="kisiNO" value="<?php echo $KisiNo; ?>">				
				<center id="sonuc_kart"></center>	
			</div>
				<div class="modal-footer">
	...
	<script>	
	$('#btnkartekle').click(function(){
		$('#yeni_KARTNO').val('');
		$("#KartEkleModal").modal('show');
		setTimeout(function(){ 
			$("#yeni_KARTNO").focus();					   
		}, 500);			
	});	
	...
	<script>
		$("#kaydetKARTFRM").submit(function (e) {
			$(':input[type="submit"]').prop('disabled', true);
			$(':input[type="submit"]').hide();
			e.preventDefault();
			$.ajax({  
				 url:"ajax.php",  
				 method:"POST",  
				 data:$('#kaydetKARTFRM').serialize(),  
				 success:function(cevap){  
					  //$('form').trigger("reset");  
					  $('#sonuc_kart').fadeIn().html(cevap);
					  setTimeout(function(){ 
						location.reload();						   
					  }, 3000);						  
				 }  
	...
	<script>	
 	...
 	<script>
		$(document).ready(function() {
			var linkDurum = "<?php echo $KisiDurum; ?>";
			if(linkDurum=="Aktif")
			{
			...
			else
			{
				...
				$('a[href="xogrenciler.php"').addClass('active');
			}
		});
	</script> 
	<script>
